/**
 * @typedef {"home" | "plus" | "utensils" | "bath" | "pencil" | "hammer" | "layers"} IconType
 */
...
/** Short one-line descriptions shown in the desktop Services dropdown. */
const serviceNavDescriptions = {
  "custom-home-design-build": "End-to-end construction from concept to completion",
  "home-additions": "Expand your living space with structural precision",
  "kitchen-remodeling": "Bespoke cabinetry, countertops, and open layouts",
  "bathroom-remodeling": "Spa-inspired finishes and custom tile work",
  "interior-exterior-design": "Cohesive design direction for every surface",
  "structural-modifications-framing": "Load-bearing walls, beams, and framing expertise",
  "basement-finishing": "Transform unused space into refined living areas",
};

/** @type {{ label: string; href: string; icon: IconType; description: string }[]} */
const serviceNavItems = servicePages.map((service) => ({
  label: service.title,
  href: `/services/${service.slug}`,
  icon: service.icon,
  description: serviceNavDescriptions[service.slug] ?? "",
}));
