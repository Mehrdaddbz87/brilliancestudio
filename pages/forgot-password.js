...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setStatus("success"); // Always show success to prevent enumeration
    } finally {
            ...
            {status === "success" ? (
              <div className="mt-8">
                ...
                <p className="mt-5 text-lg leading-8 text-text/75">
                  If an account with that email exists, a reset link has been sent to <strong className="text-text">info@brilliancestudio.ca</strong>.
                </p>
                <p className="mt-3 text-sm text-text/50">
                  The link expires in 1 hour. Check your inbox.
                </p>
                <div className="mt-8">
                  <Button href="/login" variant="ghost">Back to login</Button>
                </div>
              </div>
            ) : (
              <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  <Link href="/login" className="inline-flex items-center text-sm text-text/50 transition hover:text-accent">
                    Back to login
                  </Link>
                </div>
              </form>
